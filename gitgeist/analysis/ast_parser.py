...
class GitgeistASTParser:
    ...
    def __init__(self):
        ...
        if TREE_SITTER_AVAILABLE:
            try:
                ...
                self._init_parsers()
                logger.info(f"Tree-sitter initialized with languages: {list(self.languages.keys())}")
    ...
    def _compare_structures(self, old: Dict, new: Dict) -> Dict:
        ...
        for name in set(old_funcs.keys()) & set(new_funcs.keys()):
            if (old_funcs[name]['start_line'] != new_funcs[name]['start_line'] or
                old_funcs[name]['end_line'] != new_funcs[name]['end_line']):
