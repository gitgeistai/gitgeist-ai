...
from gitgeist.utils.logger import setup_logger, set_log_level
...
def print_logo():
    """Print Gitgeist logo"""
    console.print("🧠 [bold blue]Gitgeist[/bold blue] - Autonomous AI Git Agent", style="bold")
...
def init(
    ...
    host: str = typer.Option("http://localhost:11434", "--host", help="Ollama host URL"),
    style: str = typer.Option("conventional", "--style", "-s", 
        ...
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}")) as progress:
            ...
            if health:
                ...
                if model_available:
                    ...
                    console.print(f"Model '{model}' not found. Install with: ollama pull {model}", style="yellow")
            else:
                progress.update(task, description="❌ Ollama not responding")
                console.print("Ollama not running. Start with: ollama serve", style="red")
...
def watch():
    ...
    try:
        ...
        console.print("❌ No configuration found. Run 'gitgeist init' first.", style="red")
        raise typer.Exit(1)
...
def commit(
    ...
    auto: bool = typer.Option(False, "--auto", help="Automatically commit without confirmation")
):
    ...
    async def run_commit():
        try:
            ...
            if dry_run:
                ...
                with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}")) as progress:
                    task = progress.add_task("Generating intelligent commit message...", total=None)
                ...
                if result.get('committed'):
                    console.print(f"✅ [bold green]Committed:[/bold green] {result['message']}")
                elif result.get('message'):
                    console.print(f"💡 [bold blue]Generated message:[/bold blue] {result['message']}")
...
def status():
    ...
    async def show_status():
        try:
            ...
            table.add_row("Autonomous Mode", "🤖 Enabled" if config.autonomous_mode else "💡 Suggestion mode")
            table.add_row("LLM Model", config.llm_model)
            table.add_row("Commit Style", config.commit_style)
            
            # File changes
            total_changes = len(git_status.get('modified', [])) + len(git_status.get('added', []))
            ...
            if health:
                ...
                if model_available:
                    ...
                    console.print(f"  ⚠️  Model {config.llm_model} not found", style="yellow")
            else:
...
def config(
    ...
    if reset:
        if config_path.exists() and typer.confirm("Reset configuration to defaults?"):
            config_path.unlink()
            console.print("✅ Configuration reset. Run 'gitgeist init' to recreate.", style="green")
        return
